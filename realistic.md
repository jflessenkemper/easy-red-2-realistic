...
| # | Feature | Status | Trigger | Acts via | Log label |
|---|---|---|---|---|---|
| 19 | **Radioman calls a fire mission** when the advance stalls — the period-correct substitute for CAS | 🔧 **half built** | the phase-side **consumer** is complete: it reads the `RQ_T/RQ_X/RQ_Z/RQ_S` integer-global protocol, refuses on danger-close / cooldown / cap, then walks a WARN → FIRING state machine. The brain-side **producer** (a radioman that detects a stall and writes the request) is **not implemented**, so nothing writes `RQ_T` and only scripted `FIRE_SUPPORT_TARGETS` can fire | integer globals → `er2.explosion` | `RADIO-fire-mission accepted / REFUSED … / rounds away / complete` |
...
| 4 | Bounding | — | ⬜ not implemented; `BOUND-*` must be **absent** |
...
| 11 | AT hunting | — | ⬜ not implemented; `AT-*` must be **absent**. AT men currently fight as riflemen and are *eligible* to assault |
...
| 17 | Wounded drag | — | ⬜ not implemented; `DRAG-*` must be **absent** |
