...
| 5 | **Advance behind armour** — keep a friendly *armoured* hull between yourself and the enemy | ✅ | **attacker only** (see §7 — defenders ignore `moveTo`) · friendly vehicle within `ARMOUR_SCAN` that passes the `ARMOUR_ALLOW`/`ARMOUR_DENY` name filter, is not destroyed and is not artillery · **live recovery after the `rosterIndex` fix not yet re-confirmed in a battle (was 367 fires, regressed to 0, fix guarded by check 4d)** | `getVehiclesInArea`, `Vehicle.getName`, `isDestroyed`, `isArtilleryVehicle`, `moveTo` | `ADVANCE-behind-armour` |
...
| 18 | **Crew bail out** — crews abandon a burning or disabled vehicle and then fight as (poor) infantry | 🔧 **offline only — never observed live**, it needs a destroyed *tank* and truck occupants are passengers, not crew | `vehicle_destroyed` / `vehicle_disabled` / `vehicle_damaged_fueltank`. **Phase-side**, not a brain branch: the callbacks queue integers only, the 1 s loop drains ≤`BAIL_PER_TICK` wrecks | `getPassengers`, `getDriver`, `kickEveryoneOut` (verified per man), `leaveVehicle` fallback, `alertFor`, `say` | `bail-out: N crew left vehicle` |

## 1.4 Command, support & feedback

| # | Feature | Status | Trigger | Acts via | Log label |
|---|---|---|---|---|---|
| 19 | **Radioman calls a fire mission** when the advance stalls — the period-correct substitute for CAS | 🔧 **NOT verified end-to-end** | brain-side producer observed firing (`RADIO-fire-mission`); the phase-side consumer has **never** logged `accepted`, `REFUSED` or `rounds away` in any session. Suspected cause: the phase loop dies at the first battle boundary (§ phase-loop), so the consumer is not running by the time a request arrives | integer globals `RQ_*` → `er2.explosion` | `RADIO-fire-mission` (brain) |
| 20 | **Objective attraction manager** — pulls units onto an objective, then releases it once held so they flow to the next | ✅ | every `ATTRACT_EVERY` s, in **two passes** — ownership for *all* objectives first, attraction applied only once `allHeld` is final · **survives the engine killing the phase loop on a phase advance, via a `soldier_died` watchdog that drives the same body when the loop goes quiet** | `getAllObjectives`, `isAttractor`/`setAttractor` (guarded separately), `getSoldiersInArea`, `getVehiclesInArea` | `obj<N> … invAttract=` |
