...
| # | Feature | Base game | With Realistic |
|---|---|---|---|
| 19 | Fire support | Scripted triggers only | 🔧 A radioman whose advance has **stalled** publishes a fire-mission request; the phase script consumes it, refuses danger-close / on cooldown / over cap, then walks a WARN→FIRING state machine. **NOT verified end-to-end, and the reason is now known: the phase-side consumer runs on the phase loop, which dies at the first objective capture — so by the time a radioman has stalled long enough to call one in, the consumer is already gone. The brain side is observed firing; the phase side has never logged an accept, a refusal or a shell.** See §1 of realistic.md |
| 20 | Objective flow | Units can mill around a held objective | 🔧 Attraction manager pulls units onto an objective, then releases it once held so they flow to the next. **Works until the FIRST objective is captured, then stops: the phase-script loop reliably dies at capture (measured twice on Donchery — loop output ends while the battle runs on for another 60k log lines). No Lua error, and per-soldier brains are unaffected. Cause not yet isolated; see §Phase loop in the plugin's docs/ui-map.md.** |
