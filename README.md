...
| 18 | Crew bail-out | Crew of a dead vehicle may linger | 🔧 Ejected from burning/disabled/destroyed vehicles, alerted, and then — because a tank crew is **not** a rifle section — they break contact rather than joining the firing line. **Phase half verified live 2026-08-30 — three bail-outs, 12 men: `Sd.Kfz. 222 (disabled)` and `Opel Blitz (fuel tank hit)`, all via `kickEveryoneOut`. The brain half (`CREW-onfoot`, the dismounted crewman breaking contact) is still unobserved; with only 3 real crew and 1-in-6 decision sampling that is most likely sampling rather than failure, so it is not claimed.** |
